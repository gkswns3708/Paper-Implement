...
def train_model(rank, world_size, args):
    ...
    if master : wandb.init(project="BERT Pre-train & Fine-tuing")
    ...
    if os.path.isfile(args.save): # Resume-from의 개념.
        ...
        print(f"rank : {rank}, load_state_dict from : {args.save}, epoch = {epoch}, loss = {loss}")
    ...
    if 1 < args.n_gpu:
        ...
        model.to(config.device)
    if master: wandb.watch(model) # TODO:wandb watch api 뜻 이해하기.
    
    # TODO: Loss Function, Optimizer 등과 같은 Hyper Parameter가 실제 구현과 동일한지 비교하기
    criterion_cls = torch.nn.CrossEntropyLoss() 
    
    # 이때 sampler는 Multi-gpu를 위한 Sampling Code인 듯 합니다. 
    ...
    scheduler = optim.get_linear_scehdule_with_warmup(optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total)
    ...
    for step in trange(args.epoch, desc="Epoch"):
        ...
        if master : wandb.log({"loss" : loss, "accuracy" : score}) # TODO: F_1 score Metric을 추가해도 될 듯 하다.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train", default="./Data/ratings_train.json", type=str, required=False, help="input train file")
    parser.add_argument("--test", default="./Data/ratings_test.json", type=str, required=False, help="input test file")
    parser.add_argument("--save_path", default="./save/save_best.pth", type=str, required=False, help="Path for Saving best Performance model")
    parser.add_argument("--pretrain", default="./save/save_pretrain.pth", type=str, required=False, help="Path for pre-trained weight")
